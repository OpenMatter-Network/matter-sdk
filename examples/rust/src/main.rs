//! Seal a secret and recover it from a throwaway in-process committee; no network.
//!
//! `cargo run -p matter-sdk-example`. See `README.md` for moving to production.
...
async fn main() {
    ...
    // Stands in for committee nodes reached over HTTP.
    ...
    let secret_id = 0xc0ffee_u128; // assigned by the chain on store
    ...
    // 2. Store: submit these args with your own Substrate client.
    ...
    // 3. Decrypt: collect a threshold quorum.
...
fn run_dkg(ctx: &CrtContext<Cipher>) -> Vec<DkgOutput<Params>> {
    ...
    let shared_a = derive_shared_a::<Cipher>(SEED, EPOCH);
    // Session id binding commit-before-reveal; a real committee derives it from chain context.
