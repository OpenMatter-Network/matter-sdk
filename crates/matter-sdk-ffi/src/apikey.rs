...
//! [`msdk_apikey_free`] exactly once. The key material lives behind that handle and
...
use crate::{into_buf, MsdkBuf, MSDK_ERR_INVALID_ARG, MSDK_OK};
...
fn key_status(_: KeyError) -> i32 {
    ...
    // surface the message (or call `msdk_apikey_parse` and report generically).
    MSDK_ERR_KEY
}
...
/// Release a handle from [`msdk_apikey_parse`], wiping the key material. Passing
