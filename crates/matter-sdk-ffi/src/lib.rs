...
//! ## Ownership
//!
//! Every function that returns bytes returns an [`MsdkBuf`] the Rust side
//! allocated; the caller **must** hand it back to [`msdk_free`] (or
//! [`msdk_envelope_free`]) exactly once. Input pointers are borrowed for the call
//! only and never freed here.
//!
//! ## Errors
//!
//! Functions that can fail return an `i32`: `0` on success, negative on error
//! (see the `MSDK_ERR_*` constants). On error, any out-params are left as empty
//! [`MsdkBuf`]s (`ptr == null`, `len == 0`).
...
/// payload bytes to `*out`. Returns [`MSDK_OK`] or [`MSDK_ERR_INVALID_ARG`].
#[no_mangle]
...
/// [`MSDK_OK`], [`MSDK_ERR_INVALID_ARG`], or [`MSDK_ERR_CRYPTO`].
#[no_mangle]
pub unsafe extern "C" fn msdk_encrypt(
    ...
    // here; `msdk_encrypt` was the lone exception (audit MV-H1).
...
/// `*out_valid`. Returns [`MSDK_OK`], [`MSDK_ERR_INVALID_ARG`], or [`MSDK_ERR_CRYPTO`].
#[no_mangle]
...
/// One collected committee partial as borrowed input buffers (see [`msdk_open_secret`]).
#[repr(C)]
...
/// [`MsdkPartialInput`]. Writes the recovered plaintext to `*out` (a copy — the
/// caller owns it; treat it as sensitive). Returns [`MSDK_OK`], [`MSDK_ERR_INVALID_ARG`],
/// or [`MSDK_ERR_CRYPTO`].
#[no_mangle]
...
mod tests {
    ...
    /// Regression for audit MV-H1: `msdk_encrypt` must overwrite `*out` with empty
    ...
    /// consumer's `msdk_envelope_free` would be a wild free.
    #[test]
