...
//!
//! A member-tied key holds a `ProxyType::Scoped(ScopeSet)` proxy on its
//! member's account; the runtime admits a call iff the key's set covers the
//! call's required set.
//!
//! **Wire contract, mirrored from `matter-node`'s `common/src/scopes.rs`.** A
//! `ScopeSet` is a `u32` whose bit for `(scope, access)` is
//! `scope as u32 * 2 + access as u32`. Both enums are append-only; assigned
//! values never move. `tests::bit_layout_is_pinned` mirrors the runtime's test.
//!
//! ```
...
//! # Ok::<(), matter_sdk_key::ScopeParseError>(())
//! ```
...
/// Scope names, indexed by discriminant; shared by `Display` and `FromStr`.
...
/// How an empty set renders. Parses back, as does the empty string.
...
///
/// Construction never validates: check wire input with [`ScopeSet::is_valid`].
/// Undefined bits grant nothing on chain.
...
impl ScopeSet {
    ...
    /// A set from raw wire bits. Unvalidated; see [`Self::is_valid`].
    ...
    /// Read and Write for every listed scope.
...
impl fmt::Display for ScopeSet {
    /// `deployments:rw, secrets:r`, in discriminant order; empty is `(none)`.
...
///
/// Unlike [`KeyError`](crate::KeyError), echoes the input: scope text is never
/// key material.
...
impl FromStr for ScopeSet {
    ...
    /// Parses `deployments:rw, secrets:r`. Case-insensitive; entries separated
    /// by commas and/or whitespace. `""` and `(none)` yield [`ScopeSet::EMPTY`].
    fn from_str(s: &str) -> Result<Self, Self::Err> {
            ...
            .filter(|e| !e.is_empty())
        {
            ...
            // Reject repeated letters (`rr`) rather than folding them.
...
mod tests {
    ...
    /// Copy of matter-node's `common::scopes::tests::bit_layout_is_pinned`; keep
    /// in sync, the repos share no crate.
    #[test]
