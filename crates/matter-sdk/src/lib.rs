//! # matter-sdk
//!
//! The Rust SDK for **MatterChain** and the **OpenMatter** platform. With the
//! `chain` feature it reaches every pallet the runtime exposes; without it, it
//! covers **Secrets** — seal a secret under the matter-kgc committee and
//! recover it through a signed, threshold `/partial-decrypt` quorum.
//!
//! It wraps the pure [`matter_sdk_core`] with the three things the core
...
// Re-export the core surface so consumers need only depend on `matter-sdk`.
#[doc(inline)]
