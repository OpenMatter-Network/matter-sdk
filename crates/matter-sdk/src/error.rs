...
use matter_sdk_core::CoreError;
use matter_sdk_key::{KeyError, ScopeParseError};
use thiserror::Error;

/// Why an SDK operation failed. Each variant is a distinct cause, so callers
...
pub enum SdkError {
    ...
    /// [`KeyError`] never echoes any part of the key.
    #[error(transparent)]
    Key(#[from] KeyError),

    /// A scope string (`"deployments:w, secrets:r"`) did not parse.
    #[error(transparent)]
    ...
    /// Fewer than `threshold` nodes were reachable and healthy. Retry later or
    /// widen the node set. `faults` says why each node dropped out; surface it
    /// verbatim.
    ...
    /// The committee served a different epoch than the caller's state (a key
    /// rotation). Refetch the served epoch's `shared_a` and share commitments,
    /// then retry.
    ...
    /// The secret is sealed under a different AAD tag than the one requested.
    ...
    /// The client has no signer, so it cannot submit or authorize decryption.
    ...
    /// A member-tied API key's scopes do not cover the call. Checked before
    /// submission because the pool would only report `Inability to pay some fees`.
    ...
    NotPermitted {
        ...
        call: String,
        /// What the call needs.
        required: matter_sdk_key::ScopeSet,
        /// What the key actually has.
        held: matter_sdk_key::ScopeSet,
    },
    ...
    /// lifecycle, and roster calls a key could use to widen itself.
    ...
    /// The outer `proxy.proxy` succeeded but the wrapped call failed (reported
    /// as an event, not a dispatch error).
    ...
    /// The key's proxy was revoked or removed, confirmed by re-resolving. Not
    /// transient.
    #[error("this api key is no longer registered; it was revoked or re-scoped")]
    KeyRevoked,

    /// A delegated call was refused for fees. The key never pays: neither the
    /// member nor their billing org could cover it.
    ...
    /// A submitted extrinsic did not finalize in time. It may still land: check
    /// the chain before resubmitting, or the call may apply twice.
...
pub enum FaultStage {
    ...
    PartialDecrypt,
    /// The node served a different epoch, and too few nodes agreed to call it a
    /// rotation.
    EpochMismatch,
    /// `/health` reported a crypto protocol version this SDK does not speak.
    ProtocolVersion,
}
...
impl FaultStage {
    ...
    pub fn as_str(self) -> &'static str {
        match self {
            ...
            FaultStage::ProtocolVersion => "protocol-version",
        }
    }
}

/// Why one committee node did not contribute to a quorum. Never carries request
...
/// Faults go into `Display` so a caller that only logs `{e}` still sees them.
...
mod tests {
    ...
    /// No faults means too few nodes were supplied; no dangling separator.
    #[test]
    ...
    #[test]
    fn a_scope_parse_error_converts_with_question_mark() {
        fn parse(text: &str) -> Result<matter_sdk_key::ScopeSet> {
            Ok(text.parse()?)
        }
        assert!(parse("deployments:w").is_ok());
        assert!(matches!(parse("nonsense:w"), Err(SdkError::Scope(_))));
    }
}
