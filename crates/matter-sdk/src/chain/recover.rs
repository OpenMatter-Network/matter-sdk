//! One-call recovery of a stored secret: performs the chain reads that
//! [`crate::recover_secret`] leaves to the caller.

use matter_sdk_core::wire::AuthScheme;
...
/// order. Decoded without metadata so a test pins the layout.
#[derive(Decode)]
...
/// `pallet_kgc::KgcNodeInfo` as the chain encodes it. The epoch's committee snapshot, not
/// this index, is the authority on DKG indices.
#[derive(Decode)]
...
pub(super) struct StoredSecret {
    ...
    pub epoch: u32,
    /// The AAD the sealer committed to. Empty on older secrets; the caller must know the tag.
    pub aad: Vec<u8>,
}

/// Frames a client's key as committee-request authorization.
...
/// Refuse a tag the secret was not sealed under before any committee round trip. An empty
/// recorded AAD passes.
...
/// Join an epoch's committee with registry endpoints and share commitments, sorted by DKG
/// index.
///
/// Members without an absolute `http(s)` endpoint are dropped (the quorum tolerates `n - t`
/// missing). A kept member without its `g_j` is an error: its partial is unverifiable.
fn assemble_nodes(
...
/// `None` unless `raw` is an absolute `http(s)` URL; strips trailing `/`.
