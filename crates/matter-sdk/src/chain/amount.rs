//! Token amounts as `u128` plancks, never floats (`f64` cannot hold 18
//! decimals). Conversions take the decimal count because it comes from the live
//! runtime; see [`super::ChainProperties`].
...
///
/// Accepts `"1"`, `"1.5"`, `"0.000000000000000001"`, and a leading `+`. More
/// fractional digits than `decimals` is an error, never truncated.
///
/// ```
/// # use matter_sdk::chain::parse_amount;
...
pub fn parse_amount(text: &str, decimals: u8) -> Result<u128> {
    ...
    // Right-pad the fraction to `decimals` digits and parse as one integer.
...
///
/// ```
/// # use matter_sdk::chain::format_amount;
...
mod tests {
    ...
    fn the_decimals_discrepancy_is_a_scale_factor_of_a_million() {
        // Runtimes differ in decimals, so the exponent is never hardcoded.
