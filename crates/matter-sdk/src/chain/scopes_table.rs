...
mod tests {
    ...
    fn the_table_classifies_every_call_of_the_scoped_pallets() {
        let meta = metadata();
        // Collect every disagreement before failing: a runtime upgrade usually
        // adds several calls at once, and stopping at the first one turns a
        // single fix into as many rebuild-and-rerun cycles as there are calls.
