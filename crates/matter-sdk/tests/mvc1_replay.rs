//! Cross-node signature replay, run offline with real crypto.
//!
//! Fake nodes verify the sr25519 signature against a payload rebuilt with their own
//! index, as the live node does, before producing a real partial.
//!
//! * `v1` payload binds no node: one malicious in-subset node replays a single
//!   signature to its peers, harvests `t` partials, and opens the secret alone.
//! * `v2` payload binds the responding node's index: the replay yields one partial,
//!   never a quorum, while the honest per-node `decrypt` path still succeeds.
...
use matter_sdk::{decrypt, CommitteeNode, DecryptRequest, Health, Sr25519Signer, Transport};
use matter_sdk_core::wire::{
    from_0x,
    secret_id_to_hex,
    to_0x,
...
/// The `v1` `/partial-decrypt` payload (no responding-node binding). The SDK never
/// produces it; it exists here only to model the replay.
...
enum Scheme {
    /// Verify the v1 payload (no node binding).
    V1,
    /// Verify the v2 payload bound to the node's own index.
    V2,
}

/// DKG output + sealed secret, shared by reference (the crypto types are not `Clone`).
struct Fixture {
...
/// Honest nodes that verify the request signature before producing a real partial.
/// A failed verification is a per-node fault, mirroring the live 401.
...
impl Transport for VerifyingCommittee<'_> {
    ...
    fn partial_decrypt(
        ...
        async move {
            resp.ok_or_else(|| {
                matter_sdk::SdkError::Signer("node rejected the request signature".into())
            })
        }
    }
}

/// Signs `payload` into the wire `(requester, signature)` pair, as `Sr25519Signer` does.
...
/// A malicious in-subset node replays its one authorized signature to every subset
/// node. Returns the plaintext if that yields a quorum.
///
/// Under `V2` the held signature is the one made for the attacker itself (`subset[0]`).
fn replay_attack(
    ...
    // Lagrange coefficients are public, so the attacker computes them locally.
...
async fn recipient_binding_defeats_the_replay_attack() {
    ...
    // v1: one replayed signature harvests a quorum.
    ...
    // v2: every peer rejects the replay.
    ...
    // v2: the honest per-node signing path still opens it.
