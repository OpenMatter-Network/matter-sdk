//! The scope contract (`ScopeSet` bit layout and each call's required set), pinned as
//! fixtures every binding replays. It is ported per language, not exposed via the
//! crypto-only FFI.
//!
//! Regenerate with:
//!
...
use matter_sdk::chain::scopes_table::required_scopes;
use matter_sdk::chain::Value;
use matter_sdk::{Access, Scope, ScopeSet};
use serde_json::json;
...
/// Calls whose required set also depends on their arguments; the fixture pins only the
/// name-derived half, so bindings must test these separately.
...
fn the_bit_layout_matches_the_runtime_contract() {
    // Restates the table in matter-node/docs/api-keys.md.
...
fn emit_scope_vectors() {
    ...
    // Truth table so a binding cannot pass with `contains` as `bits != 0`.
    ...
    // `arg_sensitive` rows carry the fail-safe (wider) set used when arguments are unreadable.
