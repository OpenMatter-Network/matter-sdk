...
- **Never re-implement the cryptography.** All cryptographic logic lives in
  `matter-sdk-core` (which wraps `matter-crypto`). A language binding *calls* the core;
...
- **Cross-language parity is enforced by vectors.** If you change a core output, regenerate
  the [conformance vectors](testvectors/) (`cargo test -p matter-sdk-core --test roundtrip
