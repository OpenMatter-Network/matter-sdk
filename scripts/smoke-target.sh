#!/usr/bin/env bash
# Run one consumer smoke test for one row of scripts/native-targets.json, on this machine
# or in the row's container image (the platform's real libc).
#
...
# Oldest and newest CPython the abi3 wheel claims.
readonly -a PYPA_INTERPRETERS=(/opt/python/cp39-cp39/bin/python /opt/python/cp313-cp313/bin/python)
# The built binary must then run on BARE_IMAGE with nothing added.
readonly ALPINE_BUILD_PACKAGES='bash build-base git'
readonly BARE_IMAGE=alpine:3

# A directory spec is mounted; a version passes through. ${mount[@]+...}: bash 3.2
