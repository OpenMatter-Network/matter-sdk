#!/usr/bin/env bash
# Table tests for scripts/release-plan.sh and scripts/check-versions.sh --expect.
# A wrong answer is irreversible on a public registry.
set -euo pipefail
cd "$(dirname "$0")/.."
...
readonly -a CASES=(
  ...
  # Dispatch: same plan, never publishes.
  ...
  # Off a tag: the manifests' version.
