#!/usr/bin/env bash
# Print what PyPI already holds for a directory of wheels at one version: ABSENT, PARTIAL
# or SAME (scripts/lib-registry-state.sh). Fails on DIFFERENT. Read-only.
#
# An upload of several wheels can stop part-way, and PyPI never accepts a filename twice.
# The upload action's `skip-existing` would paper over that, and over a genuine conflict
# too. So this proves, per file, that whatever is already there is byte-identical; only
# then may the publish job skip it and upload the rest.
#
...
# Only a 404 means "not there". A 5xx or a timeout is not evidence of absence.
case "$status" in
