#!/usr/bin/env bash
# Assert a directory holds exactly the wheels we mean to upload (PyPI filenames are final).
#
#   usage: scripts/check-wheel.sh <wheels-dir> <pep440-version> [--partial]
#     --partial   allow missing platforms (local runs); present and unexpected files still fail
#
...
readonly ABI_TAG=cp39-abi3 # CPython >= 3.9
...
readonly -a REQUIRED_METADATA=(
  ...
  'Provides-Extra: sdk'
  'Project-URL: '
  # chain.py imports scalecodec directly.
  'Requires-Dist: scalecodec'
)
# Shared libs the extension may load. glibc: the manylinux-guaranteed set. musl: libc only
# (bare Alpine has no libgcc_s).
