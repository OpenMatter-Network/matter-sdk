...
// The extrinsic layout is a consensus contract: a wrong byte yields "1010 outdated"
// from the node, or a valid signature over the wrong payload.
...
// The live runtime's signed extensions, in metadata order.
...
// syntheticMetadata builds V14 metadata with only the signed-extension list,
// the one part PrepareExtrinsic reads.
...
func TestPrepareMatchesTheVerifiedRuntimeLayout(t *testing.T) {
	// Pins a byte layout the runtime is known to accept.
...
func TestNoSignedExtensionsIsAnError(t *testing.T) {
	// Empty means metadata failed to load, not that the runtime has no extensions.
