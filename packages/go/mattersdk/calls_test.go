...
// GrantTarget is a SCALE enum; a wrong variant index or payload grants access to
// the wrong principal or yields undecodable call data.
...
func TestGrantTargetVariantIndicesMatchTheRuntimeEnum(t *testing.T) {
	// `enum GrantTarget { User(AccountId), Deployment(u128) }`: declaration order
	// is the SCALE index.
	if GrantUser != 0 {
...
func TestGrantTargetRefusesToEncodeAnUnknownKind(t *testing.T) {
	// Must not fall back to User, which would grant to whatever is in Account.
