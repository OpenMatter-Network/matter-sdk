...
// Call builders for pallet-secrets.
...
// GrantTarget is who a secret is granted to: the runtime's GrantTarget enum, not a
// bare account id.
...
// RevokeAccessCall builds `Secrets.revoke_access(secret_id, target)`. The target
// must match the grant exactly, or the call is a no-op on chain.
