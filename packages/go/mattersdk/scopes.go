...
// testvectors/required_scopes.json. Ported rather than bound through cgo so
// argument-sensitive rows can read call arguments as Go values.
...
// scopeNames is indexed by Scope; rendering and parsing share it.
...
// ScopeSet is a set of (scope, access) grants, as a u32 bitmask. Immutable:
// every combinator returns a new set.
...
func ParseScopeSet(text string) (ScopeSet, error) {
	...
	for _, entry := range strings.FieldsFunc(trimmed, func(r rune) bool {
		...
		for _, c := range strings.ToLower(access) {
			// Reject a repeated letter rather than folding it.
			switch {
			case c == 'r' && !read:
				read = true
			case c == 'w' && !write:
...
// billingWrite is the Budgets calls a key may make with billing:w. Every other
// Budgets call is never admitted: the roster calls (so a key never mints
...
var deploymentsWrite = map[string]bool{
	...
	"set_deployment_volumes":        true,
	"set_deployment_restart_policy": true,
}

// RequiredScopes reports what pallet.call requires of a delegated key, and
// whether any set admits it. false means no key may ever make the call:
// provider-signed, root-only, org lifecycle, roster, and treasury value movers.
//
// # This check is a courtesy, not a boundary
//
// The runtime's filter is the only enforcer; this turns its fee-error refusal
// into "your key lacks volumes:w". By name alone, the argument-sensitive rows
// take the wider set; requiredScopesForCall reads their arguments.
...
// requiredScopesForCall is RequiredScopes with the two argument-sensitive Jobs
// rows resolved from the encoded call (clearing a secret ref needs no Secrets
// scope). Anything it cannot read takes the wider set.
...
// Option::None; for request_deployment, whether both secret references in the
// request are None. The registry drops variant names, so only an exact byte 0
// reads as None; any other shape counts as a reference.
func referencesNoSecret(fields registry.DecodedFields, field string) bool {
	for _, f := range fields {
		...
		// Both must be seen and None; a changed shape is not evidence of no secret.
