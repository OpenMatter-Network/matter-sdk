...
// Test doubles for the orchestration suite. open_secret.json carries real partials
// from a real quorum, so replaying them drives the full decrypt path offline.
...
// nodes builds the roster; each endpoint encodes its index so the fake transport
// can answer with that node's partial.
...
type fakeTransport struct {
	...
	// Per-endpoint: healthErrAt fails /health; refusePartialAt passes /health and
	// then refuses the partial request.
	healthErrAt     map[string]error
	refusePartialAt map[string]error
	// versionAt sets the /health crypto protocol version; 0 is what a node that
	// predates the field reports.
	versionAt map[string]uint16
...
func (f *fakeTransport) Health(endpoint string) (Health, error) {
	...
	return Health{Status: "active", Epoch: f.fx.Epoch, CryptoProtocolVersion: f.versionAt[endpoint]}, nil
}
...
// recordingSigner returns well-formed auth and records what it was asked to
// authorize.
