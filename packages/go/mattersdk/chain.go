...
type ChainClient struct {
	...
	meta *types.Metadata
	// calls decodes arguments for argument-sensitive scope rows; nil if the
	// registry could not be built.
...
func NewChainClient(url string) (*ChainClient, error) {
	...
	// Without a registry the scope check falls back to the wider by-name set
	// (degraded, never wrong), so a failure here does not fail the connect.
...
// NextSecretID reads the `Secrets.NextSecretId` counter (u128, little-endian).
//
// Do NOT use it to predict a store's id: concurrent submitters race for it. Read
// the id from the `Secrets.SecretStored` event; see FindStoredSecret.
...
// ExtrinsicSigner signs extrinsic payloads for SubmitCall. An oversized payload
// arrives already blake2b-hashed (see UnsignedExtrinsic.SigningPayload), so an
// implementation returns a raw sr25519 signature and must not hash again.
type ExtrinsicSigner interface {
	AccountID() []byte
	SignExtrinsic(payload []byte) ([]byte, error)
}

// KeyringSigner adapts a GSRPC KeyringPair to ExtrinsicSigner.
...
// SignExtrinsic signs with the pair's URI. `signature.Sign` hashes only payloads
// over 256 bytes, and an oversized payload arrives as a 32-byte digest, so it is
// signed verbatim as the runtime expects.
func (k KeyringSigner) SignExtrinsic(payload []byte) ([]byte, error) {
	return signature.Sign(payload, k.Pair.URI)
}

// SubmitCall signs and submits any call, returning the transaction hash. Signed
// extensions come from runtime metadata; see extrinsic.go.
...
// assemble signs `call` and returns the hex-encoded extrinsic. SubmitAndWatch
// keeps these exact bytes to find the extrinsic in its block.
...
// SubmitAndWatch signs and submits `call`, waits for finalization, and returns
// its events. A timeout <= 0 means two minutes; on timeout it returns
// KindFinalityTimeout.
func (c *ChainClient) SubmitAndWatch(signer ExtrinsicSigner, call types.Call, timeout time.Duration) (TxReceipt, extrinsicOutcome, error) {
	...
	var txHash string
	if err := c.api.Client.Call(&txHash, "author_submitExtrinsic", encoded); err != nil {
		// Pre-block refusals (e.g. an unfunded key) land here; keep the cause.
