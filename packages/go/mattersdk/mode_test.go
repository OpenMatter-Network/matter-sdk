...
// metadataWithAgentKeyCall builds V14 metadata declaring
// Budgets.authorize_agent_key, the marker of a runtime spec >= 322.
...
func TestALookupFailureStopsResolutionRatherThanGoingDirect(t *testing.T) {
	// Resolving to Direct would sign every write as the key's balance-less account,
	// failing as an unexplained fee error.
...
func TestTheResolvedModeIsLoggedAsSS58(t *testing.T) {
	// SS58 is what the dashboard showed when the key was minted.
