...
Most callers want :class:`~matter_sdk.client.MatterClient`, which wraps this with
...
Requires ``substrate-interface`` (``pip install matter-sdk[sdk]``).
"""
...
def api_key_signer(key) -> ApiKeySigner:
    """Adapt an :class:`~matter_sdk.apikey.ApiKey` for extrinsic signing.
...
class ChainClient:
    ...
    def keypair_from_seed(seed: str) -> Keypair:
        ...
        * ``create_from_seed`` ignores junctions entirely, so it is only used for a
        ...
        :class:`~matter_sdk.apikey.ApiKey`.
        """
