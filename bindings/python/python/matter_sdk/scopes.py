"""Read/Write scopes for member-tied API keys.

Port of ``matter_sdk_key::scopes``, which documents the wire contract; pinned by
``testvectors/scope_bits.json`` and ``testvectors/required_scopes.json``. Ported
rather than bound so argument-sensitive rows can read call arguments before encoding.
"""
...
#: Scope names, indexed by value; shared by rendering and parsing.
_NAMES = (
    "deployments",
    "collaborations",
    "secrets",
    ...
    def parse(cls, text: str) -> "ScopeSet":
        ...
        for entry in trimmed.replace(",", " ").split():
            ...
            for char in access.lower():
                # A repeated letter is rejected, not folded.
...
def required_scopes(
    ...
    ``None`` means no set admits it: provider-signed, root-only, org lifecycle,
    the roster calls, and every treasury value mover.

    **A courtesy, not a boundary:** the runtime's filter is the enforcer. Where an
    argument cannot be read, the wider set is required, so any error is on the
    safe side.
    """
    params = params or {}
    ...
    if pallet == "Jobs":
        ...
        if call == "set_deployment_secret_ref":
            # Absent is unreadable (wider set); explicit ``None`` is
            # ``Option::None`` (narrow set).
...
def _is_none(value: Any) -> bool:
    ...
    Both ``None`` and ``{"None": None}`` count; anything else is present.
    """
    if value is None:
        return True
...
def _request_references_secret(request: Any, present: bool) -> bool:
    ...
    Anything unreadable, including an absent argument or field, counts as
    referencing one.
    """
