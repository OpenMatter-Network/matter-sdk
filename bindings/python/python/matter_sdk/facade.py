...
as a :class:`~matter_sdk.chain.ChainError` at the call rather than silently. The
...
class SecretsFacade(_Facade):
    ...
    def grant(self, secret_id: int, target: dict):
        ...
        :func:`~matter_sdk.calls.grant_to_deployment` — the chain's
...
class KeysFacade(_Facade):
    ...
    **These are member-signed.** A key can never call them on itself — the
    ...
    def authorize(self, key: Any, scopes: Any):
        ...
        ``scopes`` may be a :class:`~matter_sdk.scopes.ScopeSet` or its raw
