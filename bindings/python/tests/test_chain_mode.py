"""``ChainClient.agent_key``: a runtime without scoped keys (read from metadata)
must be told apart from a lookup that failed (an error)."""
...
def test_a_registered_key_decodes_its_principal_and_scopes():
    # Option tag + 32-byte principal + ScopeSet as little-endian u32.
    scopes = 0x000FFFFF
